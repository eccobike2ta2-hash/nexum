...
window.NexumCart = (function() {
  ...
  // A partir de ahora SOLO se cobra el envío a través del sitio.
  // El precio de los productos es referencial (precio de mercado) y se
  // confirma/ajusta al momento de la entrega, no se cobra por aquí.
  function getTotal() {
    return getShipping();
  }
  ...
  function render() {
    ...
    if (checkoutBtn) {
      checkoutBtn.onclick = (e) => {
        ...
        mensaje += `🛵 *Envío a pagar: $${getShipping().toFixed(2)}*\n`;
        mensaje += `💰 *Total a confirmar ahora: $${getTotal().toFixed(2)} (solo envío)*\n\n`;
