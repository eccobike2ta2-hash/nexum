...
async function cargarProductosDesdeSheets() {
  try {
    ...
    window.NEXUM_PRODUCTS = filas.map(fila => {
      ...
      // Columna I (índice 8) = visible: "si" / "no"
      // Si la columna no existe o está vacía se asume visible
    ...
    renderizarCategorias();   // Primero las categorías dinámicas
    renderizarCatalogo();     // Luego los productos
...
function renderizarCategorias() {
  ...
  // Obtenemos categorías únicas solo de productos visibles
  ...
  // Construimos el HTML: primero "Todos", luego las categorías únicas
  const html = `
